...
module.exports = {
  ...
  executeInteraction: async(interaction) => {
    ...
    var missingPermissions = await interaction.client.meili.index('listing').search('', { filter: ['serverId IS NOT NULL', ['status=99', 'isStale = true']], attributesToRetrieve: ['serverId','name','owner','inviteLink'] })
...
**Completed**: ${completedRegistrations}`;
    ...
    if (missingPermissions) {
      ...
      for (const server of missingPermissions) {
        ...
        embedMessage += `\n${server.name} (${server.serverId}) ${ownerText}`
      }
    }
